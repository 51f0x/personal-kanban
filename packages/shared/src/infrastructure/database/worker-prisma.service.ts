...
import { resolveWorkerPrismaClient } from './prisma-client-resolver';
...
function getPrismaClientClass(): typeof import('@prisma/client').PrismaClient {
    if (!PrismaClientClass) {
        PrismaClientClass = resolveWorkerPrismaClient();
    }
 ...
 * Provides database access for Worker container using WORKER_DATABASE_URL
 */
@Injectable()
export class WorkerPrismaService implements OnModuleInit, OnModuleDestroy {
    ...
    constructor(private readonly configService: ConfigService) {
        // Try WORKER_DATABASE_URL first, fallback to DATABASE_URL for backward compatibility
        const databaseUrl =
            configService.get<string>('WORKER_DATABASE_URL') ||
            configService.get<string>('DATABASE_URL');
        if (!databaseUrl) {
            throw new Error('WORKER_DATABASE_URL or DATABASE_URL is not configured');
        }
        ...
        return new Proxy(this, {
            get(target, prop: string | symbol) {
                // If it's a property of WorkerPrismaService itself, return it
