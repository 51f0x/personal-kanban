...
export abstract class BaseAgent {
    ...
    constructor(config: ConfigService, loggerContext: string) {
        ...
        this.endpoint = config.get<string>('LLM_ENDPOINT', 'http://localhost:11434');
        this.model = config.get<string>('LLM_MODEL', 'granite4:1b');
        this.llmTimeoutMs = config.get<number>('LLM_TIMEOUT_MS', 60000); // 60s default
     ...
     */
    protected async callLlm<T>(generateFn: () => Promise<T>, context: string): Promise<T> {
    ...
    protected validateAndTruncateContent(
        ...
        this.logger.warn(`Content truncated from ${content.length} to ${maxLength} characters`);
