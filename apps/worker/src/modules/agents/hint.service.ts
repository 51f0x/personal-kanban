...
import { AgentProcessingResult } from './types';
...
export class HintService {
        ...
        try {
            ...
            if (results.taskAnalysis?.success) {
                ...
                if (
                    ...
                    results.taskAnalysis.suggestedTags.length > 0
                ) {
                    hints.push({
                        taskId,
                        agentId: 'task-analyzer-agent',
            ...
            if (results.contextExtraction?.success) {
                ...
                if (
                    results.contextExtraction.projectHints &&
                    results.contextExtraction.projectHints.length > 0
                ) {
                    hints.push({
                        taskId,
                        agentId: 'context-extractor-agent',
            ...
            // Note: Markdown conversion happens AFTER hints are auto-applied
            // Markdown hint is created in task-processor.service.ts after conversion
    ...
    private async autoApplyHighConfidenceHints(taskId: string, hintIds: string[]): Promise<void> {
        try {
            ...
            this.logger.log(
                `Auto-applying ${highConfidenceHints.length} high-confidence hints (>=80%) for task ${taskId}: ${highConfidenceHints.map((h) => `${h.hintType} (${(h.confidence || 0) * 100}%)`).join(', ')}`,
            );
            ...
            switch (hint.hintType) {
                ...
                case 'actions':
                    if (hint.data && typeof hint.data === 'object' && 'actions' in hint.data) {
                        const actions =
                            (hint.data as { actions?: Array<{ description: string }> }).actions ||
                            [];
                        if (actions.length > 0) {
