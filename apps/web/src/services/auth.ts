...
export async function login(credentials: LoginCredentials): Promise<AuthUser> {
  ...
  // Store accessToken for JWT auth (used by apiRequest for all subsequent requests)
  if (data.accessToken) {
    localStorage.setItem(AUTH_TOKEN_KEY, data.accessToken);
  }

  // Store refreshToken for token renewal flow
...
export async function signup(
  ...
  if (loginData.refreshToken) {
    localStorage.setItem(AUTH_REFRESH_TOKEN_KEY, loginData.refreshToken);
  }
