import { Mail, Users, Phone, BookOpen, Video, Monitor, MoreHorizontal, X } from 'lucide-react';
...
const contextConfig: Record<TaskContext, { label: string; icon: typeof Mail; color: string }> = {
  ...
  MEETING: { label: 'Meeting', icon: Users, color: 'bg-purple-100 text-purple-700 hover:bg-purple-200' },
  PHONE: { label: 'Phone', icon: Phone, color: 'bg-green-100 text-green-700 hover:bg-green-200' },
  READ: { label: 'Read', icon: BookOpen, color: 'bg-amber-100 text-amber-700 hover:bg-amber-200' },
  WATCH: { label: 'Watch', icon: Video, color: 'bg-red-100 text-red-700 hover:bg-red-200' },
  DESK: { label: 'Desk', icon: Monitor, color: 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200' },
  OTHER: { label: 'Other', icon: MoreHorizontal, color: 'bg-slate-100 text-slate-700 hover:bg-slate-200' },
};

const allContexts: TaskContext[] = ['EMAIL', 'MEETING', 'PHONE', 'READ', 'WATCH', 'DESK', 'OTHER'];

export function ContextFilterBar({ selectedContexts, onContextToggle, onClearFilters }: ContextFilterBarProps) {
