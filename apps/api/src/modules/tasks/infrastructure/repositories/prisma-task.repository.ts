...
export class PrismaTaskRepository implements ITaskRepository {
    ...
    async findById(id: TaskId, options?: FindTasksOptions): Promise<TaskWithRelations | null> {
    ...
    async findByBoardId(
        ...
        const tasks = await this.prisma.task.findMany({
            ...
            include: Object.keys(include).length > 0 ? include : undefined,
            orderBy: [{ columnId: 'asc' }, { position: 'asc' }, { lastMovedAt: 'asc' }],
        });
    ...
    async update(id: TaskId, data: Partial<TaskData>): Promise<TaskData> {
        ...
        if (data.needsBreakdown !== undefined) updateData.needsBreakdown = data.needsBreakdown;
        ...
        if (data.lastMovedAt !== undefined) updateData.lastMovedAt = data.lastMovedAt;
    ...
    async countByColumnId(columnId: ColumnId, excludeTaskId?: TaskId): Promise<number> {
    ...
    async updatePositions(updates: Array<{ id: TaskId; position: number }>): Promise<void> {
