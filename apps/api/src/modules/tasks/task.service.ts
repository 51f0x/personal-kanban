...
export class TaskService {
    ...
    return this.prisma
      ...
      .then(async (task) => {
        ...
        await this.eventBus.publish(
          new TaskUpdatedEvent(taskId, boardId, updateData),
        );
        return task;
      });
  }
  ...
  async moveTask(id: string, input: MoveTaskDto): Promise<MoveTaskResult> {
    ...
    if (
      !(await this.columnRepository.belongsToBoard(targetColumnId, taskBoardId))
    ) {
    ...
    const updatedTask = await this.prisma
      .$transaction(async (tx) => {
        ...
        const updated = await tx.task.update({
          ...
          data: {
            ...
            completedAt:
              targetColumn.type === "DONE" ? new Date() : task.completedAt,
          },
        });
      ...
      .then(async (updated) => {
        ...
        const taskId = TaskId.from(id);
        const boardId = BoardId.from(task.boardId);
        const fromColumnIdVO = fromColumnId
          ? ColumnId.from(fromColumnId)
          : null;
   ...
   */
  async markStale(id: string, isStale = true) {
    const taskId = TaskId.from(id);
    ...
    return this.prisma
      ...
      .then(async (task) => {
        ...
        if (isStale) {
          ...
          const boardId = BoardId.from(task.boardId);
          await this.eventBus.publish(
            new TaskStaleEvent(taskId, boardId, isStale),
          );
        }
        return task;
      });
