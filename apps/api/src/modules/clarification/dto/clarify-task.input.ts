...
export class ClarifyTaskDto {
    ...
    @ApiPropertyOptional({ description: 'Step 8: Needs breakdown into subtasks?', example: false })
    @IsOptional()
    @IsBoolean()
    needsBreakdown?: boolean;

    @ApiPropertyOptional({
        description: 'Non-actionable routing: where should non-actionable items go?',
