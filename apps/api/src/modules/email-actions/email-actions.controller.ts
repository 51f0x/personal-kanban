...
export class EmailActionsController {
  ...
  @ApiOperation({
    ...
    description:
      "Marks a task as complete using a secure token from an email link",
  })
  @ApiQuery({
    name: "token",
    description: "Secure token from email link",
    required: true,
  })
  @ApiResponse({
    status: 302,
    description: "Redirects to task view after completion",
  })
  ...
  async completeTask(@Query("token") token: string) {
    ...
    try {
      ...
      this.logger.log(
        `Task ${result.taskId} completed via email link, redirecting to ${redirectUrl}`,
      );
  ...
  @ApiOperation({
    ...
    description:
      "Redirects to task view using a secure token from an email link",
  })
  @ApiQuery({
    name: "token",
    description: "Secure token from email link",
