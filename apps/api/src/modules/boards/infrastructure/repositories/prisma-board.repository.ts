...
export class PrismaBoardRepository implements IBoardRepository {
    ...
    async findById(id: BoardId, options?: FindBoardsOptions): Promise<BoardWithRelations | null> {
    ...
    async findByOwnerId(
        ...
        return boards.map((board) => this.mapToBoardData(board) as BoardWithRelations);
    }
    ...
    async create(board: Omit<BoardData, 'id' | 'createdAt' | 'updatedAt'>): Promise<BoardData> {
