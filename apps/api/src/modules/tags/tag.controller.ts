...
export class TagController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "List tags for board",
    description: "Get all tags for a specific board",
  })
  ...
  @ApiOperation({
    summary: "Create tag",
    description: "Create a new tag for a board",
  })
  @ApiParam({ name: "boardId", description: "Board ID" })
  @ApiBody({
    type: CreateTagDto,
    description: "Tag data (boardId is taken from URL parameter)",
  })
  @ApiResponse({ status: 201, description: "Tag created successfully" })
  @ApiResponse({ status: 400, description: "Invalid input" })
  createTag(
    @Param("boardId") boardId: string,
    @Body() dto: Omit<CreateTagDto, "boardId">,
  ) {
