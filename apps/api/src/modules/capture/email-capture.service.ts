...
export class EmailCaptureService {
    ...
    async processEmail(dto: EmailWebhookDto) {
        ...
        if (!user) {
            this.logger.warn(`Email capture rejected: sender ${senderEmail} not found in system`);
        ...
        if (!user.defaultBoardId || !user.defaultBoard) {
            this.logger.warn(`Email capture rejected: user ${user.id} has no default board`);
            throw new BadRequestException(
        ...
        const targetColumn = board.columns.find((col) => col.type === 'INPUT') ?? board.columns[0];

        if (!targetColumn) {
            this.logger.error(`Email capture failed: board ${board.id} has no columns`);
        ...
        this.logger.log(`Created task ${task.id} from email sent by ${senderEmail}`);
