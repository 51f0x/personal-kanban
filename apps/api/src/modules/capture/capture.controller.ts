...
export class CaptureController {
  ...
  @ApiOperation({
    ...
    description:
      "Quickly add a task via capture endpoint. Requires capture token authentication.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Capture a task from an incoming email. The sender's email must match a registered user, and the task will be created on their default board.",
  })
  ...
  @ApiResponse({
    status: 400,
    description: "Invalid input or sender email not registered",
  })
  @ApiResponse({ status: 401, description: "Invalid capture token" })
