...
export class AgentCaptureService {
  ...
  private async processTaskWithAgents(taskId: string, boardId: string): Promise<void> {
    try {
      ...
      // Emit error progress
      this.agentProgressService.emitAgentProgress(
        taskId,
        boardId,
        {
          taskId,
  ...
  private async processTaskWithUrl(
    ...
    try {
      // Get API base URL for progress callbacks
      const apiBaseUrl = this.config.get<string>('API_URL', 'http://localhost:3000');
      const progressCallbackUrl = `${apiBaseUrl}/api/v1/agents/progress/update`;
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`Failed to queue agent processing for task ${task.id}: ${errorMessage}`);
