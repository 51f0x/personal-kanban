...
export class InterContainerQueueService
  implements IInterContainerQueue, OnModuleInit
{
  ...
  constructor(
    ...
    this.responseQueue = new Queue<ApiResponse>("api-responses", {
      connection,
    });
  }
  ...
  async onModuleInit() {
    ...
    this.worker.on(
      "completed",
      async (job: Job<ApiRequest>, result: ApiResponse) => {
  ...
  async request<TRequest extends ApiRequest, TResponse extends ApiResponse>(
    ...
    throw new Error(
      "request() should be called from worker container, not API",
    );
  }

  async send<TRequest extends ApiRequest>(
    _queue: string,
    _request: TRequest,
  ): Promise<void> {
