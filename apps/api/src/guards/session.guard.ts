    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        try {
            ...
            if (result) {
                ...
                // Ensure user is set
                if (!request.user) {
                    this.logger.error('Authentication succeeded but user not set on request');
                    return false;
                }
                ...
                await new Promise<void>((resolve, reject) => {
                    request.login(request.user, (err?: Error) => {
                        if (err) {
                            ...
                            this.logger.debug(`Session created for user: ${request.user.email}`);
                            resolve();
                        }
                    });
                });
            ...
            if (error instanceof Error) {
                this.logger.error(`SessionGuard authentication failed: ${error.message}`, error.stack);
            } else {
                this.logger.error(`SessionGuard authentication failed: ${String(error)}`);
            }
